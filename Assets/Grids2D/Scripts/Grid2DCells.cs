	...
	public partial class Grid2D : MonoBehaviour {
		...
		public int rowCount { 
			...
			set {
				if (value != _cellRowCount) {
					value = Mathf.Clamp(value, 2, MAX_SIZE);
					_cellRowCount = value;
					isDirty = true;
					GenerateMap ();
				}
		...
		public GameObject CellToggle (int cellIndex, bool visible, Color color, bool refreshGeometry, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace, bool useCanvasRect = false) {
			...
			if (surf != null) {
				...
				surfMaterial = surf.GetComponent<Renderer> ().sharedMaterial;
				if ((texture == null && !surfMaterial.name.Equals (coloredCellMat.name)) || (texture != null && !surfMaterial.name.Equals (texturizedCellMat.name))
				    || (surfMaterial.color != color && !isHighlighted) || (texture != null && (region.customMaterial == null || region.customMaterial.mainTexture != texture))) {
					Material goodMaterial = GetColoredTexturedMaterialForCell(color, texture);
					region.customMaterial = goodMaterial;
					ApplyMaterialToSurface (surf, goodMaterial);
				}
			} else {
				surfMaterial = GetColoredTexturedMaterialForCell(color, texture);
		...
		public void CellSetTexture (int cellIndex, Texture2D texture) {
			...
			if (texture != null) {
				if (cell.region.customMaterial == null) {
					cell.region.customMaterial = GetColoredTexturedMaterialForCell(Color.white, texture);
				}
				if (cellSurface != null) {
		...
		public GameObject CellSetSprite (int cellIndex, Color tintColor, Sprite sprite) {
			...
			if (surf != null) {
				...
				// Check if material is ok
				surfMaterial = surf.GetComponent<Renderer> ().sharedMaterial;
				if (!surfMaterial.name.Equals (texturizedCellMat.name) || (surfMaterial.color != tintColor && !isHighlighted) || ((region.customMaterial == null || region.customTextureOffset != sprite.textureRectOffset || region.customMaterial.mainTexture != sprite.texture))) {
					Material goodMaterial = GetColoredTexturedMaterialForCell(tintColor, sprite.texture);
					region.customMaterial = goodMaterial;
					ApplyMaterialToSurface (surf, goodMaterial);
				}
			} else {
				surfMaterial = GetColoredTexturedMaterialForCell (tintColor, sprite.texture);
