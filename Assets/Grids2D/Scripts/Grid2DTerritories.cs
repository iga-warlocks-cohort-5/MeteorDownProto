...
namespace Grids2D
{
	...
	public partial class Grid2D : MonoBehaviour
	{
		...
		/// </summary>
		public Vector3 TerritoryGetPosition(Territory territory, float elevation = 0) {
			int territoryIndex = TerritoryGetIndex(territory);
			return TerritoryGetPosition(territoryIndex, elevation);
		}
		...
		public GameObject TerritoryToggle (int territoryIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool useCanvasRect = false)
		{
			...
			if (surf != null) {
				if (texture != null && (region.customMaterial == null || textureScale != region.customTextureScale || textureOffset != region.customTextureOffset ||
				                textureRotation != region.customTextureRotation || !region.customMaterial.name.Equals (texturizedTerritoryMat.name))) {
					surfaces.Remove (cacheIndex);
			...
			if (surf != null) {
				...
				if ((texture == null && !surfMaterial.name.Equals (coloredTerritoryMat.name)) || (texture != null && !surfMaterial.name.Equals (texturizedTerritoryMat.name))
				                || (surfMaterial.color != color && !isHighlighted) || (texture != null && region.customMaterial.mainTexture != texture)) {
					Material goodMaterial = GetColoredTexturedMaterialForTerritory(color, texture);
					region.customMaterial = goodMaterial;
					ApplyMaterialToSurface (surf, goodMaterial);
				}
			} else {
				surfMaterial = GetColoredTexturedMaterialForTerritory (color, texture);
