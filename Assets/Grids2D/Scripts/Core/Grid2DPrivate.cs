...
namespace Grids2D {
    ...
    public partial class Grid2D : MonoBehaviour {
        ...
        const double MIN_VERTEX_DIST = 0.0002;
        const double SQR_MIN_VERTEX_DIST = MIN_VERTEX_DIST * MIN_VERTEX_DIST;
        Rect canvasRect = new Rect(-0.5f, -0.5f, 1, 1);

        // Custom inspector stuff
        public const int MAX_TERRITORIES = 256;
        public const int MAX_CELLS = 40000;
        ...
        Material coloredCellMat, texturizedCellMat;
        Material coloredTerritoryMat, texturizedTerritoryMat;
        ...
        Dictionary<Color, Material> coloredMatCacheForCells;
        Dictionary<Color, Material> coloredMatCacheForTerritories;
        Color[] factoryColors;
        ...
        public void Init() {
            ...
            if (coloredCellMat == null) {
                coloredCellMat = Instantiate(Resources.Load<Material>("Materials/ColorizedCellRegion"));
                coloredCellMat.MarkForDisposal();
            }
            if (texturizedCellMat == null) {
                texturizedCellMat = Instantiate(Resources.Load<Material>("Materials/TexturizedCellRegion"));
                texturizedCellMat.MarkForDisposal();
            }
            coloredMatCacheForCells = new Dictionary<Color, Material>();
            if (coloredTerritoryMat == null) {
                coloredTerritoryMat = Instantiate(Resources.Load<Material>("Materials/ColorizedTerritoryRegion"));
                coloredTerritoryMat.MarkForDisposal();
            }
            if (texturizedTerritoryMat == null) {
                texturizedTerritoryMat = Instantiate(Resources.Load<Material>("Materials/TexturizedTerritoryRegion"));
                texturizedTerritoryMat.MarkForDisposal();
            }
            coloredMatCacheForTerritories = new Dictionary<Color, Material>();

            if (_surfacesTypeOrder == SURFACES_TYPE_ORDER.CellsOverTerritories) {
                coloredCellMat.renderQueue++;
                texturizedCellMat.renderQueue++;
            } else {
                coloredTerritoryMat.renderQueue++;
                texturizedTerritoryMat.renderQueue++;
            }
            UpdateMaterialProperties();
        ...
        void UpdateMaterialProperties() {
            ...
            UpdateRenderQueue(coloredCellMat);
            UpdateRenderQueue(texturizedCellMat);
            UpdateRenderQueue(coloredTerritoryMat);
            UpdateRenderQueue(texturizedTerritoryMat);
        }
        ...
        Material GetColoredTexturedMaterialForCell(Color color, Texture2D texture) {
            if (texture == null && coloredMatCacheForCells.ContainsKey(color)) {
                return coloredMatCacheForCells[color];
            } else {
                Material customMat;
                if (texture != null) {
                    customMat = Instantiate(texturizedCellMat);
                    customMat.name = texturizedCellMat.name;
                    customMat.mainTexture = texture;
                } else {
                    customMat = Instantiate(coloredCellMat);
                    customMat.name = coloredCellMat.name;
                    coloredMatCacheForCells[color] = customMat;
                }
        ...
        Material GetColoredTexturedMaterialForTerritory(Color color, Texture2D texture) {
            if (texture == null && coloredMatCacheForTerritories.ContainsKey(color)) {
                return coloredMatCacheForTerritories[color];
            } else {
                Material customMat;
                if (texture != null) {
                    customMat = Instantiate(texturizedTerritoryMat);
                    customMat.name = texturizedTerritoryMat.name;
                    customMat.mainTexture = texture;
                } else {
                    customMat = Instantiate(coloredTerritoryMat);
                    customMat.name = coloredTerritoryMat.name;
                    coloredMatCacheForTerritories[color] = customMat;
                }
                customMat.color = color;
                customMat.MarkForDisposal();
                return customMat;
